...
1)  Все слова из нашего текста оказались не длиннее 31 символа, а значить каждое из них влезет в 256 бит. Будем использовать в качестве ключей хэш-таблицы `union`.
...
**Вес не оптимизированных функций не больше 0.02% поэтому дальнейшие оптимизации этой хэш-таблицы нецелесообразны, потому что прирост производительности будет очень меньше 1%.
...
- в хэш-функции `rol(hash(n - 1)) * int(word[n])` использована ассемблерная вставка. Оптимизация неудачна, производительность осталась на прежнем уровне.
